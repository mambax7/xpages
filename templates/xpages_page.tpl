    ...
    <article class="xpages-article">
		...
		<section class="xpages-extra-fields">
			...
			<{foreach key=fname item=f from=$xpages_page.extra_fields}>
				...
				<div class="xpf xpf-<{$f.field_type}>" id="xpf-<{$fname}>">
					<div class="xpf-label"><strong><{$f.field_label}>:</strong></div>
					<div class="xpf-value">
						<{if $f.field_type == 'file'}>
							<{* Dosya/Resim gösterimi *}>
							<{assign var="fileExt" value=$f.value|substr:-3|lower}>
							<{if $fileExt == 'jpg' || $fileExt == 'png' || $fileExt == 'gif' || $fileExt == 'peg' || $fileExt == 'webp'}>
								<img src="<{$f.value}>" alt="<{$f.field_label}>" style="max-width:100%;max-height:300px;border-radius:5px">
							<{else}>
								<a href="<{$f.value}>" target="_blank" rel="noopener">📎 Dosyayı İndir</a>
							<{/if}>
						...
						<{elseif $f.field_type == 'checkbox'}>
							<{if $f.value == 1 || $f.value == '1' || $f.value == 'on' || $f.value == 'yes'}>✓ Evet<{else}>✗ Hayır<{/if}>
...
<section class="xpages-gallery">
    ...
    <div class="xpages-gallery-grid" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(200px,1fr));gap:15px;margin-top:15px">
        ...
        <div class="xpages-gallery-item" style="border-radius:8px;overflow:hidden;box-shadow:0 2px 5px rgba(0,0,0,0.1)">
            <a href="<{$g.image_url}>" data-lightbox="xpages-gallery" data-title="<{$g.title}>">
                <img src="<{$g.image_url}>" alt="<{$g.title}>" style="width:100%;height:150px;object-fit:cover">
            </a>
